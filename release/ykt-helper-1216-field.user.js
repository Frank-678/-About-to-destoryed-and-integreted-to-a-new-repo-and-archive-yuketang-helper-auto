...
(function() {
  ...
  function clone$1(value) {
    if (Array.isArray(value)) return value.map(clone$1);
    if (value && typeof value === "object") return Object.fromEntries(Object.entries(value).map(([key, item]) => [ key, clone$1(item) ]));
    return value;
  }
  ...
  function normalizeProfiles(saved = {}, ai = {}, legacyKimiApiKey = "") {
    let profiles;
    if (validProfiles(ai.profiles)) profiles = clone$1(ai.profiles); else if (validProfiles(saved.profiles)) profiles = clone$1(saved.profiles); else if (validProfiles(DEFAULT_CONFIG.profiles)) profiles = clone$1(DEFAULT_CONFIG.profiles); else profiles = [];
    const legacyKey = String(legacyKimiApiKey || ai.kimiApiKey || ai.apiKey || "");
    if (!profiles.length) profiles.push({
      ...
      visionModel: "moonshot-v1-8k-vision-preview",
      temperature: ""
    }); else if (legacyKey && !profiles.some(profile => String(profile?.apiKey || "").trim())) profiles[0] = {
      ...profiles[0],
      apiKey: legacyKey
    };
  ...
  function createGMPrivateStore() {
    return {
      available() {
        return !!runtimeFunction("GM_getValue") && !!runtimeFunction("GM_setValue") && !!runtimeFunction("GM_deleteValue");
      },
      get(key, dv = null) {
  ...
  function hasSecretMaterial(config, separateLegacyKey = "") {
    ...
    return !!secrets.legacyKimiApiKey || !!secrets.ocrApiKey || !!secrets.translateApiKey || Object.values(secrets.profileApiKeys).some(Boolean);
  }
  ...
  // src/core/storage.js
    ...
    constructor(prefix, {privateStore: privateStore = null} = {}) {
      this.prefix = prefix;
    ...
    _privateAvailable() {
      try {
        return !!this.privateStore?.available?.();
      } catch {
        return false;
      }
    }
    _readPrivateSecrets() {
      ...
      try {
        return normalizeSecretRecord(this.privateStore.get(this.privateSecretsKey, null));
      } catch {
        return null;
      }
    }
    ...
    _removeLocalLegacyKey() {
      try {
        localStorage.removeItem(this.prefix + "kimiApiKey");
      } catch {}
    }
    ...
    get(key, dv = null) {
      ...
      // GM storage. Never scrub the old copy until the private write succeeds.
            if (this._privateAvailable() && hasSecretMaterial(value, legacyKimiApiKey)) try {
    ...
    remove(key) {
      if (key === "kimiApiKey" && this._privateAvailable()) {
        ...
        if (current) {
          ...
          try {
            this._writePrivateSecrets(current);
          } catch {}
        }
      }
  ...
  // src/state/repo.js
    ...
    activeLessons: new Map,
    // lessonId -> 最近一次 on-lesson API 记录
    ...
    isLessonConnected(lessonId) {
      ...
      if (socket.readyState !== void 0 && typeof WebSocket !== "undefined") if (socket.readyState === WebSocket.CLOSED || socket.readyState === WebSocket.CLOSING) return false;
      return socket;
    },
    markLessonDisconnected(lessonId, reason = "closed", expectedSocket = null) {
      ...
      // lesson state when a caller provides the socket it is cleaning up.
            if (expectedSocket && currentSocket !== expectedSocket) return {
        lessonId: key,
        reason: "stale-owner",
  ...
  function normalizeOperation$1(value) {
  ...
  function operationFrom(source) {
    for (const key of OPERATION_KEYS) {
      ...
      const normalized = normalizeOperation$1(value);
  ...
  function getLessonId$1(message, context = {}) {
    return firstText(context, [ "lessonId", "lesson_id", "lessonid" ]) || findNestedText(message, [ "lessonId", "lesson_id", "lessonid" ]);
  }
  function getPresentationId(message, category, entityId) {
    return category === "courseware" ? entityId : findNestedText(message, [ "presentationId", "presentation_id", "presentationid", "pres" ]);
  }
  ...
  function classifyPublishEvent(message, context = {}) {
    ...
    const lessonId = getLessonId$1(message, context);
  ...
  function isCurrentPublishEvent(event, {currentLessonId: currentLessonId = null, currentPresentationId: currentPresentationId = null} = {}) {
    ...
    const presentationId = event.presentationId || (event.category === "courseware" ? event.entityId : null);
  ...
  function getRealtimeEvent(message, context = {}) {
    const op = normalizeOp$1(message);
   ...
   */  function dispatchRealtimeMessage(message, {getRuntimeMode: getRuntimeMode = () => "desktop", lessonId: lessonId = null, handlers: handlers = {}} = {}) {
    const eventContext = lessonId !== void 0 && lessonId !== null && String(lessonId) !== "" ? {
    ...
    if (realtime?.kind === "timeline" || realtime?.kind === "unlockproblem" || realtime?.kind === "danmu") options.source = realtime.kind === "timeline" ? "timeline" : "live";
  ...
  function findDanmuText(source, visited = new Set) {
    ...
    visited.add(source);
    for (const key of [ "danmu", "barrage", "text" ]) if (typeof source[key] === "string" && source[key].trim()) return source[key];
  ...
  function isDanmuSendMessage(message) {
    ...
    if (!(operation.includes("danmu") || operation.includes("barrage") || operation === "sendmessage" || operation === "sendchat")) return false;
   ...
   */  function confirmDanmuSend(message) {
    if (!isDanmuSendMessage(message)) return 0;
    const outboundText = findDanmuText(message);
    const index = pendingSends.findIndex(item => item.text === outboundText || item.text.trim() === String(outboundText).trim());
    if (index === -1) return 0;
  ...
  /** Send one text through the native classroom barrage controls. */  async function sendDanmuText(text, {root: root = globalThis.document, awaitConfirmationMs: awaitConfirmationMs = DEFAULT_CONFIRMATION_MS} = {}) {
    ...
    try {
      ...
      const timeout = Math.max(0, Number.isFinite(Number(awaitConfirmationMs)) ? Number(awaitConfirmationMs) : DEFAULT_CONFIRMATION_MS);
    ...
    MyWebSocket.addHandler((ws, url) => {
      ...
      if (routeLessonId && !pendingManagedLessonId) {
        ...
        // never close another native socket here.
                if (previousSocket && previousSocket !== ws && previousSocket.__yktManaged === true) try {
      ...
      // 接收侧统一分发
        ...
        try {
          ...
          if (!dispatched.handled) console.log("[雨课堂助手][WARN] 未知WebSocket操作:", message?.op || message?.type || null);
          // 监听后端传递的url
    ...
    ws.addEventListener("open", () => {
      try {
        ...
        console.log("[雨课堂助手][INFO][AutoJoin] 已发送 hello 握手:", {
          lessonId: String(lessonId),
          useridPresent: hello.userid !== void 0 && hello.userid !== null,
          authPresent: !!auth
        });
      } catch (e) {
    ...
    MyXHR.addHandler((xhr, method, url) => {
      ...
      // 课件：精确路径或包含关键字
        ...
        xhr.intercept(resp => {
          ...
          if (resp && (resp.code === 0 || resp.success)) runtimeActionRef.current?.onPresentationLoaded(id, resp.data || resp.result);
        });
        return;
      }
      // 答题
        ...
        xhr.intercept((resp, payload) => {
          try {
            ...
            if (resp && (resp.code === 0 || resp.success)) runtimeActionRef.current?.onAnswerProblem(problemId, result);
          } catch (e) {
      ...
      if (url.pathname === "/api/v3/lesson/problem/retry") {
        xhr.intercept((resp, payload) => {
          try {
            ...
            if (resp?.code === 0 && first?.problemId) runtimeActionRef.current?.onAnswerProblem(first.problemId, first.result);
          } catch {}
        });
        return;
      }
 ...
 // 一定要返回原始 Response
  ...
  function lessonSetHas(setLike, lessonId) {
    ...
    // Compare normalized values so policy remains stable across upgrades/reloads.
        if (typeof setLike[Symbol.iterator] === "function") for (const value of setLike) if (normalizeLessonId(value) === key) return true;
    return false;
  }
  function shouldAutoAnswerForLesson$1({lessonId: lessonId, config: config, autoJoinedLessons: autoJoinedLessons, forceAutoAnswerLessons: forceAutoAnswerLessons} = {}) {
    if (config?.autoAnswer === true) return true;
    const key = normalizeLessonId(lessonId);
    if (!key) return false;
    if (config?.autoAnswerOnAutoJoin === true && lessonSetHas(autoJoinedLessons, key)) return true;
    return lessonSetHas(forceAutoAnswerLessons, key);
  }
  /** Select the server route for one answer attempt. */  function chooseAnswerRoute({now: now = Date.now(), endTime: endTime = null, forceRetry: forceRetry = false, allowRetryAfterDeadline: allowRetryAfterDeadline = false} = {}) {
    if (forceRetry) return "retry";
    if (endTime === null || endTime === void 0 || endTime === "") return "answer";
    const current = Number(now);
    const deadline = Number(endTime);
    if (allowRetryAfterDeadline && Number.isFinite(current) && Number.isFinite(deadline) && current >= deadline) return "retry";
    return "answer";
  }
   ...
   */  async function retryAnswer(problem, result, dt, options = {}) {
    ...
    if (!confirmed) throw new Error("服务器未返回成功信息");
    return resp;
  }
  /**
   ...
   */  async function submitAnswer(problem, result, submitOptions = {}) {
    ...
    const autoAnswerEnabled = shouldAutoAnswerForLesson$1({
      lessonId: lessonId,
      config: ui?.config,
  ...
  function getActiveProfile$1(aiCfg, profileId = null) {
    const cfg = aiCfg || {};
  ...
  function withProfileTemperature(profile, payload) {
    ...
    if (!Number.isFinite(temperature) || temperature < 0 || temperature > 2) throw new Error("Temperature 必须是 0 到 2 之间的数字，或留空使用模型默认值");
    return {
      ...requestPayload,
 ...
 // 单选题
   ...
   */  function isWithinAnswerPriorityWindow(now = Date.now(), windows = []) {
    ...
    for (const item of windows) {
      ...
      const {beforeMs: beforeMs, afterMs: afterMs} = getWindowBounds(item);
   ...
   */  function selectAnswerProfile(aiConfig, {role: role = "active", now: now = Date.now(), windows: windows = [], fastProfileId: fastProfileId = aiConfig?.fastAnswerProfileId, verifyProfileId: verifyProfileId = aiConfig?.verifyAnswerProfileId} = {}) {
    const profiles = getProfiles(aiConfig);
    const active = getActiveProfile(profiles, aiConfig?.activeProfileId);
    if (!active) return null;
    if (role === "fast" && isWithinAnswerPriorityWindow(now, windows)) return profiles.find(profile => profile.id === fastProfileId) || active;
    if (role === "verify") return profiles.find(profile => profile.id === verifyProfileId) || active;
    return active;
  }
  ...
  /** Compare two parsed answers while ignoring formatting-only differences. */  function compareParsedAnswers(problem, first, second) {
   ...
   */  const RECOVERY_STORAGE_PREFIX = "auto-answer-recovery:";
  const DEFAULT_RECOVERY_TTL_MS = 24 * 60 * 60 * 1e3;
  const DEFAULT_RECOVERY_MAX_ENTRIES = 100;
  ...
  function normalizeRecoveryRecord(input = {}, {lessonId: lessonId, now: now = Date.now()} = {}) {
   ...
   */  function shouldRecoverProblem(record, {enabled: enabled = false, recoverExpired: recoverExpired = false, now: now = Date.now()} = {}) {
    if (!enabled) return {
      recover: false,
      forceRetry: false,
      reason: "disabled"
    ...
    if (!PENDING_PHASES.has(record.phase)) return {
      recover: false,
      forceRetry: false,
      reason: "not-pending"
    };
    if (record.autoAnswerQueued === false && record.phase !== "answering") return {
      recover: false,
      forceRetry: false,
      reason: "not-queued"
    };
   ...
   */  function createProblemRecoveryStore({storage: storage, lessonId: lessonId, now: now = () => Date.now(), ttlMs: ttlMs = DEFAULT_RECOVERY_TTL_MS, maxEntries: maxEntries = DEFAULT_RECOVERY_MAX_ENTRIES} = {}) {
    ...
    function prune() {
      ...
      let changed = false;
      for (const [problemId, record] of records) if (record.lessonId !== lesson || hasTtl && current - record.updatedAt > ttlMs) {
      ...
      if (records.size > limit) {
        const oldest = [ ...records.values() ].sort((a, b) => a.updatedAt - b.updatedAt).slice(0, records.size - limit);
    ...
    function load() {
      ...
      for (const input of entries) {
        ...
        const record = normalizeRecoveryRecord(input, {
    ...
    function upsert(input = {}) {
      ...
      const record = normalizeRecoveryRecord({
        ...existing,
        ...input,
        updatedAt: input.updatedAt ?? Number(now())
      }, {
        lessonId: lesson,
        now: Number(now())
      });
  ...
  function createAutoAnswerRunner({typeMap: typeMap = {}, hasActiveProfile: hasActiveProfile = () => false, getAIConfig: getAIConfig = () => {}, getAnswerProfile: getAnswerProfile = () => null, makeDefaultAnswer: makeDefaultAnswer = () => null, captureSlideImage: captureSlideImage = async () => null, captureProblemForVision: captureProblemForVision = async () => null, formatProblemForVision: formatProblemForVision = () => "", queryAIVision: queryAIVision = async () => "", parseAIAnswer: parseAIAnswer = () => null, submitAnswer: submitAnswer = async () => ({
    route: "answer"
  }), onAnswered: onAnswered, onStatusChange: onStatusChange, notify: notify, toast: toast, showPopup: showPopup, verifyAnswer: verifyAnswer = null, now: now = () => Date.now()} = {}) {
    async function run(problem, status, {force: force = false, forceRetry: forceRetry = false, allowResubmit: allowResubmit = false, source: source = (force ? "manual" : "auto"), lessonId: lessonId = null} = {}) {
      if (!problem || !status) return {
        ok: false,
      ...
      if (expired && !force && !forceRetry) return {
        ok: false,
        reason: "expired"
      };
      ...
      try {
        ...
        if (!activeAIProfile) parsed = makeDefaultAnswer(problem); else {
          ...
          if (!image) image = await captureProblemForVision();
        ...
        if (typeof verifyAnswer === "function" && aiContent) try {
          ...
          if (verificationState === "corrected" && verification?.answer !== void 0) try {
          ...
          } else if (verification?.aiAnswer !== void 0) finalAIContent = verification.aiAnswer;
        } catch (error) {
        ...
        const successDetail = verificationState === "corrected" ? "快速答案已提交，验证模型发现差异并完成修正。" : shouldRetry ? "答案已强制补交。" : void 0;
        notify?.("auto-answer-succeeded", problem, successDetail, {
          source: source,
          submission: submission,
          verificationState: verificationState
        });
        toast?.(shouldRetry ? "AI 作答完成并已补交" : "AI 作答完成", 3e3);
        ...
        toast?.(`AI 作答失败：${status.lastError}`, 4e3);
        return {
          ok: false,
          reason: "error",
          error: error
  ...
  function parseEditableAnswer(problemType, rawValue) {
    ...
    try {
      ...
      if (parsed !== null && typeof parsed === "object") {
        if ([ 1, 2, 3 ].includes(problemType) && Array.isArray(parsed)) return parsed.length ? parsed : null;
        if (problemType === 4 && Array.isArray(parsed)) return parsed.length ? parsed : null;
        if (problemType === 5 && typeof parsed.content === "string" && parsed.content.trim()) return {
  ...
  function formatEditableAnswer(value) {
    try {
  ...
  function buildAnswerSubmitOptions(status = {}, {lessonId: lessonId = null, forceRetry: forceRetry = false} = {}) {
    return {
  ...
  const DANMU_FOLLOW_DEFAULTS = DEFAULTS;
   ...
   */  function createDanmuRoundTracker(options = {}) {
   ...
   */  function createDanmuFollowTracker(options = {}) {
    ...
    const burstWindowMs = finitePositive(options.burstWindowMs ?? options.windowMs, DEFAULTS.burstWindowMs);
    const roundGapMs = finitePositive(options.roundGapMs, DEFAULTS.roundGapMs);
    const maxSendsPerRound = Math.max(0, Math.floor(Number.isFinite(Number(options.maxSendsPerRound)) ? Number(options.maxSendsPerRound) : DANMU_FOLLOW_DEFAULTS.maxSendsPerRound));
    let messages = [];
    let lastAt = null;
    let roundStartedAt = null;
    let sentCount = 0;
   ...
   */  function createDanmuFollowController(options = {}) {
    ...
    async function handle(message, {notificationOnly: notificationOnly = false} = {}) {
      ...
      const finalize = resolved => {
        if (resolved === true || resolved?.sent === true && resolved?.verified !== false) rememberOwn(observation.text, now);
        return {
          handled: true,
          ...enriched,
          sendResult: resolved
      ...
      if (sendResult && typeof sendResult.then === "function") return sendResult.then(finalize, error => finalize({
        sent: false,
        text: observation.text,
        reason: "send-error",
        error: error
   ...
   */  function syncActiveLessons(previous = [], records = []) {
    const active = [];
    const seen = new Set;
  ...
  const LATEST_TIME_KEYS = [ "startTime", "start_time", "startAt", "start_at", "beginTime", "begin_time", "lessonStartTime", "lesson_start_time", "createdAt", "created_at" ];
  function toTimestamp(value) {
    if (value instanceof Date) return value.getTime();
    if (typeof value === "number" && Number.isFinite(value)) return Math.abs(value) < 1e12 ? value * 1e3 : value;
    const text = String(value ?? "").trim();
    if (!text) return null;
    if (/^-?\d+(?:\.\d+)?$/.test(text)) {
      const number = Number(text);
      return Math.abs(number) < 1e12 ? number * 1e3 : number;
    }
  ...
  /** Return the newest status=1 lesson, preserving API order when no time exists. */  function pickLatestActiveLesson(records) {
    const active = (Array.isArray(records) ? records : []).filter(isActiveLesson).filter(record => getLessonId(record) !== null);
   ...
   */  function createNavigationArbiter({waitMs: waitMs = 1e4, setTimeoutFn: setTimeoutFn = setTimeout, clearTimeoutFn: clearTimeoutFn = clearTimeout, navigate: navigate = () => {}} = {}) {
    let timer = null;
    ...
    return {
      offer: offer,
      observeUserIntent: observeUserIntent,
      reset: reset,
  ...
  function shouldAutoAnswerForLesson(lessonId) {
    return shouldAutoAnswerForLesson$1({
      lessonId: lessonId,
      config: ui?.config,
      autoJoinedLessons: repo?.autoJoinedLessons,
      forceAutoAnswerLessons: repo?.forceAutoAnswerLessons
 ...
 // 多选
  ...
  function problemIdKey(problemId) {
    return problemId === void 0 || problemId === null ? null : String(problemId);
  }
  function getProblemById(problemId) {
    const key = problemIdKey(problemId);
    if (!key) return null;
    return repo.problems.get(problemId) || repo.problems.get(key) || repo.problems.get(Number.isNaN(Number(key)) ? key : Number(key)) || null;
  }
  function getProblemStatus(problemId) {
    const key = problemIdKey(problemId);
    if (!key) return null;
    return repo.problemStatus.get(problemId) || repo.problemStatus.get(key) || repo.problemStatus.get(Number.isNaN(Number(key)) ? key : Number(key)) || null;
  }
  function getProblemRecoveryStore(lessonIdOverride = null) {
    ...
    if (!_problemRecoveryStore || _problemRecoveryLessonId !== key) {
      ...
      _problemRecoveryStore = createProblemRecoveryStore({
        storage: storage,
        lessonId: key
      });
    }
  ...
  function statusPhase(status) {
    if (status?.phase === "queued" || status?.phase === "answering" || status?.phase === "failed") return status.phase;
  ...
  function hasActiveAIProfile(aiCfg, selectedProfile = null) {
  ...
  function selectAutoAnswerProfile(role = "active", now = Date.now()) {
    ...
    // 快速 Profile 没有密钥时回退到当前 Profile，避免因误配而改用默认答案。
      ...
      const active = selectAnswerProfile(aiConfig, {
        role: "active",
        now: now
      });
  ...
  async function verifyAutoAnswer({problem: problem, image: image, prompt: prompt, firstAnswer: firstAnswer, firstRawAnswer: firstRawAnswer, now: now}) {
    if (ui.config.answerVerification !== true) return {
      answer: firstAnswer,
      aiAnswer: firstRawAnswer,
      state: "disabled"
    };
    const verifyProfileId = String(ui.config.verifyAnswerProfileId || "").trim();
    if (!verifyProfileId) return {
      answer: firstAnswer,
      aiAnswer: firstRawAnswer,
      state: "not-configured"
    };
    const verifyProfile = selectAutoAnswerProfile("verify", now);
    if (!verifyProfile?.apiKey || String(verifyProfile.id) !== verifyProfileId) return {
      answer: firstAnswer,
      aiAnswer: firstRawAnswer,
      state: "unavailable"
    };
    ...
    const verificationPrompt = [ prompt, "", "【快速模型候选答案】", JSON.stringify(firstAnswer), "", "请使用当前课件图片和题干独立复核候选答案。不要盲从候选答案；如果候选答案错误，请给出你认为正确的答案。", "必须按原题型要求输出，并包含“答案:”字段；只输出最终答案和必要的简短解释。" ].join("\n");
    const verifiedRawAnswer = await queryAIVision(image, verificationPrompt, ui.config.ai, {
      profileId: verifyProfile.id,
      problemType: problem.problemType
    });
    const verifiedAnswer = parseAIAnswer(problem, verifiedRawAnswer);
    if (!verifiedAnswer) return {
      answer: firstAnswer,
      aiAnswer: firstRawAnswer,
      state: "invalid"
    };
    if (compareParsedAnswers(problem, firstAnswer, verifiedAnswer)) return {
      answer: firstAnswer,
  ...
  const autoAnswerRunner = createAutoAnswerRunner({
    ...
    getAnswerProfile: ({role: role, now: now}) => selectAutoAnswerProfile(role, now),
  ...
  const actions = {
    ...
    onUnlockProblem(data, {notificationOnly: notificationOnly = false, source: source = "live", lessonId: lessonId = null, liveRetryCount: liveRetryCount = 0} = {}) {
      ...
      const autoAnswerEnabled = shouldAutoAnswerForLesson(lessonId || previous?.lessonId || recovered?.lessonId || problem?.lessonId || repo.currentLessonId);
      ...
      status.autoAnswerQueued = isFirstUnlock ? isLiveUnlock && autoAnswerEnabled : status.autoAnswerQueued !== false;
      if (isLiveUnlock && autoAnswerEnabled && !status.done) status.autoAnswerQueued = true;
    ...
    onPublishEvent(event, {lessonId: lessonId = null} = {}) {
      ...
      if (notified) console.log("[雨课堂助手][INFO][Publish] 已提醒发布事件:", contextualEvent.category, contextualEvent.dedupeKey);
      return notified;
    },
    ...
    onLessonFinished({lessonId: lessonId = null} = {}) {
      ...
      return ui.notifyClassroomEvent({
        ...
        title: "下课提示",
        detail: eventLessonId && eventLessonId !== repo.currentLessonId ? `课堂 ${eventLessonId} 已结束。` : "当前课程已结束。"
      });
    },
 ...
 // 已有连接
          ...
          if (repo.autoJoinRunning && generation === _autoJoinGeneration) _autoJoinTimer = setTimeout(loop, 5e3);
        }
      };
      void loop();
    },
    ...
    installRouterRearm() {
      ...
      const rearm = ({userIntent: userIntent = true} = {}) => {
    ...
    startAutoClickOnOnLessonBar() {
      ...
      const cancelAutomaticNavigation = event => {
        ...
        if (_navigationArbiter.observeUserIntent()) console.log("[雨课堂助手][INFO][AutoJoin] 检测到用户操作，取消自动跳转");
        if (_autoJumpRetryTimer !== null) {
          clearTimeout(_autoJumpRetryTimer);
          _autoJumpRetryTimer = null;
        }
      ...
      for (const eventName of [ "pointerdown", "mousedown", "touchstart", "keydown", "wheel" ]) doc.addEventListener(eventName, cancelAutomaticNavigation, {
        capture: true,
        passive: eventName === "wheel" || eventName === "touchstart"
      });
      async function offerLatestClassroom() {
        if (_autoOnLessonClickInProgress || _navigationArbiter.userHasIntent || _navigationArbiter.hasPending) return false;
        ...
        try {
          ...
          if (offered) console.log("[雨课堂助手][INFO][AutoJoin] 已找到最新活跃课堂，等待十秒确认:", lessonId);
          return offered;
        } catch (error) {
  ...
  function parsePriorityTimes(value) {
    ...
    for (const time of raw) {
      ...
      windows.push({
        at: `${String(Number(match[1])).padStart(2, "0")}:${match[2]}`
      });
    }
    return windows;
  }
  // ------------------------------
    ...
    const $autoRecoverUnanswered = root$4.querySelector("#ykt-input-auto-recover-unanswered");
    const $autoRecoverExpired = root$4.querySelector("#ykt-input-auto-recover-expired");
    const $autoScanUnanswered = root$4.querySelector("#ykt-input-auto-scan-unanswered");
    const $autoForceRetry = root$4.querySelector("#ykt-input-auto-force-retry");
    const $delay = root$4.querySelector("#ykt-input-answer-delay");
    const $rand = root$4.querySelector("#ykt-input-random-delay");
    const $answerPriorityTimes = root$4.querySelector("#ykt-input-answer-priority-times");
    const $fastAnswerProfile = root$4.querySelector("#ykt-ai-fast-profile");
    const $answerVerification = root$4.querySelector("#ykt-input-answer-verification");
    const $verifyAnswerProfile = root$4.querySelector("#ykt-ai-verify-profile");
    const $verificationDelay = root$4.querySelector("#ykt-input-verification-delay");
    ...
    const $notifyDanmuRoundStart = root$4.querySelector("#ykt-input-notify-danmu-round-start");
    const $notifyDanmuFollowTrigger = root$4.querySelector("#ykt-input-notify-danmu-follow-trigger");
    ...
    function refreshAnswerProfileSelects() {
      const ai = ui.config.ai;
      const configs = [ [ $fastAnswerProfile, "", "不使用快速模型", ui.config.fastAnswerProfileId ], [ $verifyAnswerProfile, "", "不使用复核模型", ui.config.verifyAnswerProfileId ] ];
    ...
    function syncFormFromConfig() {
      ...
      $answerPriorityTimes.value = (Array.isArray(ui.config.answerPriorityWindows) ? ui.config.answerPriorityWindows : []).map(item => typeof item === "string" ? item : item?.at || item?.time || "").filter(Boolean).join(", ");
      ...
      $verificationDelay.value = Math.floor((ui.config.answerVerificationDelay || 0) / 1e3);
    ...
    // 保存设置
      ...
      ui.config.answerVerificationDelay = Math.max(0, Math.min(60, +$verificationDelay.value || 0)) * 1e3;
      refreshAnswerProfileSelects();
  ...
  function isProblemAnswered(problem = {}, status = {}) {
  ...
  function getFiniteDeadline(...values) {
  ...
  function isProblemExpired(now = Date.now(), ...deadlineValues) {
    const endTime = getFiniteDeadline(...deadlineValues);
    return endTime !== void 0 && Number(now) >= endTime;
  }
 ...
 // 只允许手动路径进入
  ...
  async function submitEditedAnswer() {
    ...
    try {
      ...
      ui.toast(expired ? "编辑答案已补交" : "编辑答案已提交", 3e3);
    } catch (error) {
  ...
  async function forceAIAnswerForCurrent() {
    ...
    try {
      ...
      ui.toast(expired ? "AI 作答完成并已补交" : "AI 作答完成", 3e3);
    } catch (error) {
 ...
 // ★ 标记为已完成
  ...
  function updateSlideView() {
    ...
    if (slide.problem) {
      ...
      forceAI.addEventListener("click", trustedUiHandler(async ev => {
        ...
        try {
          const result = await actions.forceAIAnswer(prob.problemId, {
            forceRetry: expired
          });
          if (!result?.ok) ui.toast(`AI 强制作答未完成：${result?.error?.message || result?.reason || "未知原因"}`, 4e3);
        } finally {
  ...
  function createKeyedActionLock() {
  ...
  // ========== 行渲染与交互 ==========
    ...
    btnForceAI.onclick = trustedUiHandler(async () => {
      ...
      try {
        const result = await actions.forceAIAnswer(e.problemId, {
          forceRetry: expired
        });
        if (!result?.ok) ui.toast(`AI 强制作答未完成：${result?.error?.message || result?.reason || "未知原因"}`, 4e3); else ui.toast(expired ? "AI 作答完成并已补交" : "AI 作答完成");
        updateProblemList();
      } catch (error) {
        ui.toast(`AI 强制作答失败：${error?.message || error}`, 4e3);
      } finally {
  ...
  function updateRow(row, e, prob) {
    ...
    const endTime = getFiniteDeadline(status?.endTime, prob?.endTime, e.endTime, ps?.endTime);
    // 元信息（含截止时间）
