...
(function() {
  ...
  function normalizeProfiles(saved = {}, ai = {}, legacyKimiApiKey = "") {
    let profiles;
    if (validProfiles(ai.profiles)) profiles = clone(ai.profiles); else if (validProfiles(saved.profiles)) profiles = clone(saved.profiles); else if (validProfiles(DEFAULT_CONFIG.profiles)) profiles = clone(DEFAULT_CONFIG.profiles); else profiles = [];
  ...
  // src/core/storage.js
    ...
    get(key, dv = null) {
      ...
      return normalizeRuntimeConfig(value, {
        legacyKimiApiKey: legacyKimiApiKey
      });
    }
    set(key, value) {
